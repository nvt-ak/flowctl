...
# Pin FLOWCTL_HOME to run dir so runtime files (traceability, budget-state, etc.)
# stay inside the test sandbox — avoids ~/.flowctl path confusion after v1.1 migration.
export FLOWCTL_HOME="$RUN_DIR/.flowctl"
mkdir -p "$RUN_DIR" "$FLOWCTL_HOME"

# TRACEABILITY_FILE and BUDGET_STATE_FILE are resolved AFTER init writes meta.json.
# Derive them from the flow_id that init will generate.
# Placeholder — updated by _update_runtime_paths() after flowctl init runs.
RUNTIME_DIR=""
TRACEABILITY_FILE=""
BUDGET_STATE_FILE=""

_update_runtime_paths() {
  local fl_id fl_name fl_short fl_slug _data_dir
  fl_id=$(python3 -c "import json; print(json.load(open('$STATE_FILE')).get('flow_id',''))" 2>/dev/null || echo "")
  fl_name=$(python3 -c "import json; print(json.load(open('$STATE_FILE')).get('project_name',''))" 2>/dev/null || echo "project")
  if [[ -n "$fl_id" ]]; then
    fl_short="${fl_id:3:8}"
    fl_slug=$(printf '%s' "$fl_name" | tr '[:upper:]' '[:lower:]' | tr -cs 'a-z0-9' '-' | sed 's/^[-]*//;s/[-]*$//' | cut -c1-32)
    [[ -z "$fl_slug" ]] && fl_slug="project"
    _data_dir="$FLOWCTL_HOME/projects/${fl_slug}-${fl_short}"
    ...
    # Per-flow dispatch/gate/retro dirs (mirrors config.sh DISPATCH_BASE logic)
    DISPATCH_BASE="$REPO_ROOT/workflows/$fl_short/dispatch"
    GATE_REPORTS_DIR="$REPO_ROOT/workflows/$fl_short/gates/reports"
    RETRO_DIR="$REPO_ROOT/workflows/$fl_short/retro"
  else
    # Fallback: legacy path (pre-v1.1 or no flow_id)
...
# Fresh budget policy aligned with repo template
...
cleanup() {
  ...
  rm -rf "$FLOWCTL_TEST_LOCK_DIR" 2>/dev/null || true
...
REPORT_DIR="$DISPATCH_BASE/step-1/reports"
mkdir -p "$REPORT_DIR"
...
from pathlib import Path
import os
p=Path(os.environ.get("GATE_REPORTS_DIR", "$REPO_ROOT/workflows/gates/reports")) / "gate-events.jsonl"
print(p.read_text(encoding="utf-8") if p.exists() else "")
PY
)"
