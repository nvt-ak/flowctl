...
assert_out() {
  ...
  # merge_cursor_mcp.py prints MCP_STATUS=... plus GLOBAL_MCP_STATUS=... — match a full line.
  while IFS= read -r line || [[ -n "$line" ]]; do
    [[ "$line" == "$want" ]] && return 0
  done <<< "$got"
  fail "$msg (expected line '$want', got: $got)"
}
...
python3 -c "
import json
p = json.load(open('$MCP'))
for k in ('shell-proxy', 'flowctl-state'):
    env = p['mcpServers'][k].get('env') or {}
    assert env.get('FLOWCTL_PROJECT_ROOT') == '\${workspaceFolder}', 'missing env %r' % (k,)
" || fail "scaffold MCP servers must set FLOWCTL_PROJECT_ROOT env for Cursor"
...
for k in ('shell-proxy', 'flowctl-state'):
    env = d['mcpServers'][k].get('env') or {}
    assert env.get('FLOWCTL_PROJECT_ROOT') == '\${workspaceFolder}', 'missing env %r' % (k,)
" || fail "full setup template must inject FLOWCTL_PROJECT_ROOT for Cursor"
