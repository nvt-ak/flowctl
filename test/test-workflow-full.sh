...
# Helper to run flowctl with the environment
flowctl() {
  FLOWCTL_HOME="$FLOWCTL_HOME" FLOWCTL_PROJECT_ROOT="$FLOWCTL_PROJECT_ROOT" REPO_ROOT="$REPO_ROOT" "$WORKFLOW_SCRIPT" "$@"
}
...
# Test skip
echo "Testing skip..."
flowctl skip --step 3 --reason "testing" >/dev/null
status_after_skip=$(flowctl status)
assert_contains "Step 3/9: UI/UX Design (@ui-ux)" "$status_after_skip" "step 3 should exist"
# Check that step 3 is skipped? We'll just ensure command succeeded.

# Test unskip
echo "Testing unskip..."
flowctl unskip --step 3 --reason "testing" >/dev/null
status_after_unskip=$(flowctl status)
# Should be back to pending
assert_contains "Step 3/9: UI/UX Design (@ui-ux)" "$status_after_unskip" "step 3 should exist"

# Test assess
echo "Testing assess..."
assess_output=$(flowctl assess)
assert_contains "Step" "$assess_output" "assess should show steps"

# Test version
echo "Testing version..."
version_output=$(flowctl --version)
# Just check it runs
