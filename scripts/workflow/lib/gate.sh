...
wf_write_gate_report() {
  ...
  local detail="$3"
  local actor="$4"
  local reports_dir="$GATE_REPORTS_DIR"
...
wf_evaluate_gate() {
  ...
  WF_STEP="$step" WF_DISPATCH_BASE="$DISPATCH_BASE" python3 - <<'PY'
import json, os, sys
from pathlib import Path
...
dispatch_base = Path(os.environ["WF_DISPATCH_BASE"])
...
# Skipped steps auto-pass gate — no work needed
if (step_obj.get("status") or "").strip() == "skipped":
    skip_reason = step_obj.get("skip_reason", "")
    print(f"GATE_OK|Step {step} skipped ({skip_reason}) — gate auto-pass")
    raise SystemExit(0)
...
reports_dir = dispatch_base / f"step-{step}" / "reports"
...
require_merc_outputs = bool(g.get("require_mercenary_outputs_completed", False))
...
# Mercenary output check: every spawned brief must have a matching output file
merc_dir = dispatch_base / f"step-{step}" / "mercenaries"
missing_merc_outputs = []
if merc_dir.exists():
    spawned = {f.stem.replace("-brief", "") for f in merc_dir.glob("*-brief.md")}
    completed = {f.stem.replace("-output", "") for f in merc_dir.glob("*-output.md")}
    missing_merc_outputs = sorted(spawned - completed)
if require_merc_outputs and missing_merc_outputs:
    errors.append(f"Mercenary output(s) missing: {', '.join(missing_merc_outputs)}")

if errors:
    print("GATE_FAIL|" + " ; ".join(errors))
    raise SystemExit(1)

merc_status = f" merc={len(missing_merc_outputs)}_missing" if merc_dir.exists() and (spawned := {f.stem.replace('-brief','') for f in merc_dir.glob('*-brief.md')}) else ""
print(
    f"GATE_OK|step={step} reports={report_count}({report_source}) deliverables={len(deliverables)} "
    f"decisions={len(decisions)} open_blockers={len(open_blockers)}{merc_status}"
)
PY
}
