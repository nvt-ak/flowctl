...
PY
  ...
  WF_STEP="$step" WF_STATE="$STATE_FILE" WF_REPO="$REPO_ROOT" WF_DISPATCH="$dispatch_dir" WF_REPORTS="$reports_dir" WF_POLICY_FILE="$ROLE_POLICY_FILE" WF_TRUST_REQUESTED="$trust_workspace" WF_DISPATCH_MODE="$dispatch_mode" WF_LIB_DIR="$LIB_DIR" WF_DISPATCH_BASE="$DISPATCH_BASE" python3 - <<'PY'
import json
import os
import sys
from pathlib import Path
...
try:
    ...
    # Resolve context_snapshot.py: prefer LIB_DIR (works when flowctl is installed
    # globally via npm and the script is not copied into the user repo), then fall
    # back to the legacy repo-local path for projects that ship their own copy.
    _lib_dir = os.environ.get("WF_LIB_DIR", "")
    snap_script = (Path(_lib_dir) / "context_snapshot.py") if _lib_dir else None
    if not (snap_script and snap_script.exists()):
        snap_script = repo_root / "scripts" / "workflow" / "lib" / "context_snapshot.py"
    snapshot_md = ""
    if snap_script.exists():
        try:
            snapshot_md = subprocess.check_output(
                [sys.executable, str(snap_script), str(state_path), step, str(repo_root)],
                text=True,
...
### Layer 2 — GitNexus + graph overview (code tasks only — steps 4-8)
"""
        if is_code_step:
            brief += f"""```
# GitNexus: CLI only (see .cursor/rules/tool-constraints.mdc) — e.g. gitnexus commit, gitnexus pr
# Code structure: cat graphify-out/GRAPH_REPORT.md (and graph MCP / query_graph if enabled)
```
"""
        else:
...
```
query_graph("component or flow to understand")   ← code structure only (if graph MCP enabled)
```

### Layer 4 — File reads (fallback, only when Layers 1-3 are insufficient)
"""
        if digest_rel:
...
cmd_collect() {
  ...
  local reports_dir="$DISPATCH_BASE/step-$step/reports"
...
def has_deliverable(target: str) -> bool:
    for d in deliverables:
        if isinstance(d, dict):
            if target in (d.get("claim") or "") or target in (d.get("path") or ""):
                return True
        elif target in str(d):
            return True
    return False
...
for rf in report_files:
    ...
    for line in rf.read_text(encoding="utf-8").splitlines():
        # Strip leading list marker ("- ", "* ", "+ ") before keyword matching
        # so both "DECISION: foo" and "- DECISION: foo" are handled identically.
        ...
        if s.startswith("DECISION:"):
            ...
            # "BLOCKER: NONE" is a no-op placeholder — skip it
...
idempotency_path = Path("$IDEMPOTENCY_FILE")
...
PY
  ...
  if [[ "$collect_raw" == "NO_REPORTS" ]]; then
    ...
    echo -e "Mẫu report: ${BOLD}.cursor/templates/agent-dispatch-template.md${NC}\n"
    exit 0
  fi
