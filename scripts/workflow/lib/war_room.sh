...
_war_room_generate_briefs() {
  ...
  local snap_py="${BASH_SOURCE[0]%/*}/context_snapshot.py"
  local snap_md=""
  if [[ -f "$snap_py" ]]; then
...
# GitNexus: CLI only (see .cursor/rules/tool-constraints.mdc)
cat graphify-out/GRAPH_REPORT.md     ← code structure overview for code steps
query_graph("...")                   ← optional if graph MCP enabled
\`\`\`
> Graphify has no workflow data; use for code structure only.
...
snap_path = repo_root / "scripts" / "workflow" / "lib" / "context_snapshot.py"
try:
    snap = subprocess.check_output(
        [sys.executable, str(snap_path), str(state_path), step, str(repo_root)],
        text=True,
        stderr=subprocess.DEVNULL,
    )
except Exception:
    snap = "_Context snapshot unavailable._\n"
...
{chr(10).join(prior_decisions[-10:]) if prior_decisions else "- Chưa có decisions từ các steps trước"}
