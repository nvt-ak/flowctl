...
function resolveWorkflowStatePath() {
  const resolvePy = join(dirname(__filename), '..', 'lib', 'resolve_state_path.py');
  try {
    const env = { ...process.env, REPO_ROOT, FLOWCTL_PROJECT_ROOT: REPO_ROOT };
    const out = execFileSync('python3', [resolvePy], { cwd: REPO_ROOT, env, encoding: 'utf8' });
    const j = JSON.parse(out.trim());
    if (j.state_file) return resolve(j.state_file);
  } catch {
    /* fallback */
  }
  return join(REPO_ROOT, 'flowctl-state.json');
}

const STATE_FILE = resolveWorkflowStatePath();
