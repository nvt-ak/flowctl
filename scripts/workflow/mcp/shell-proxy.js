...
function resolveWorkflowStatePath() {
  const resolvePy = join(dirname(__file), '..', 'lib', 'resolve_state_path.py');
  try {
    const out = execFileSync('python3', [resolvePy], {
      cwd: REPO,
      env: { ...process.env, REPO_ROOT: REPO, FLOWCTL_PROJECT_ROOT: REPO },
      encoding: 'utf8',
    });
    const j = JSON.parse(out.trim());
    if (j.state_file) return resolve(j.state_file);
  } catch {
    /* python missing or resolve failed — fall back */
  }
  return join(REPO, 'flowctl-state.json');
}

const STATE       = resolveWorkflowStatePath();

/** Align with scripts/workflow/lib/config.sh DISPATCH_BASE (per-flow vs legacy flat). */
function resolveDispatchBase(repo, statePath) {
  if (process.env.DISPATCH_BASE) return resolve(process.env.DISPATCH_BASE);
  try {
    const raw = existsSync(statePath) ? readFileSync(statePath, 'utf8') : '';
    const fid = (JSON.parse(raw || '{}').flow_id || '').trim();
    if (fid.length >= 11 && fid.startsWith('wf-')) {
      return join(repo, 'workflows', fid.slice(3, 11), 'dispatch');
    }
  } catch { /* ignore */ }
  return join(repo, 'workflows', 'dispatch');
}
const DISPATCH_BASE_PATH = resolveDispatchBase(REPO, STATE);

// Resolve FLOWCTL_HOME: env var → repo-local (.flowctl-local / .flowctl with projects/) → ~/.flowctl
function resolveFlowctlHome() {
  if (process.env.FLOWCTL_HOME) return process.env.FLOWCTL_HOME;
...
const FLOWCTL_HOME = resolveFlowctlHome();
const REGISTRY_F   = join(FLOWCTL_HOME, 'registry.json');

// ── Cache dir resolution ───────────────────────────────────────
// Priority:
//   1. FLOWCTL_CACHE_DIR env var (set by flowctl.sh when run via CLI)
//   2. meta.json cache_dir for this project under FLOWCTL_HOME/projects/
//      (auto-detected — no env vars needed in mcp.json)
//   3. Legacy fallback: REPO/.cache/mcp
function resolveProjectCacheDir() {
  if (process.env.FLOWCTL_CACHE_DIR) return resolve(process.env.FLOWCTL_CACHE_DIR);
  const projectsDir = join(FLOWCTL_HOME, 'projects');
  if (existsSync(projectsDir)) {
    for (const entry of readdirSync(projectsDir)) {
      ...
      try {
        ...
        if (resolve(meta.path || '') === resolve(REPO) && meta.cache_dir) {
...
function tool_step_context({ step } = {}) {
  ...
  const digestPath = join(DISPATCH_BASE_PATH, `step-${currentStep}`, 'context-digest.md');
  ...
  const wrDir   = join(DISPATCH_BASE_PATH, `step-${currentStep}`, 'war-room');
  const mercDir = join(DISPATCH_BASE_PATH, `step-${currentStep}`, 'mercenaries');

  // Collect skipped steps so workers know which steps were bypassed and why
  const skippedSteps = Object.entries(stateData.steps ?? {})
    .filter(([, ps]) => (ps.status ?? '') === 'skipped')
    .map(([n, ps]) => ({ step: Number(n), name: ps.name ?? '', reason: ps.skip_reason ?? '' }))
...
function tool_wf_reports_status({ step } = {}) {
  ...
  const reportsDir = join(DISPATCH_BASE_PATH, `step-${currentStep}`, 'reports');
  const submitted = [];
...
const TOOLS = [
  ...
  { name: 'wf_step_context',   description: 'Full step context (state, decisions, blockers, digest pointers). Prefer Context Snapshot in worker briefs/digests; use this when fresher state is needed.',          inputSchema: { type: 'object', properties: { step: { type: 'number' } } },                                                                 fn: withLogging('wf_step_context', (a) => tool_step_context(a)) },
