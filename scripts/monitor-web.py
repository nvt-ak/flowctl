...
# Respect FLOWCTL_HOME env var (exported by flowctl.sh / config.sh).
# Falls back to ~/.flowctl so plain `python3 monitor-web.py` still works.
_raw_home = _os.environ.get("FLOWCTL_HOME", "")
FLOWCTL_HOME    = (
    _normalize_path(Path(_raw_home))
    if _raw_home
    else Path.home() / ".flowctl"
)
...
def load_stats():
    try: return json.loads(STATS_F.read_text(encoding="utf-8")) if STATS_F.exists() else {}
...
def load_flow_state():
    try: return json.loads(STATE_F.read_text(encoding="utf-8")) if STATE_F.exists() else {}
    except (json.JSONDecodeError, OSError): return {}
...
def group_into_turns(events, gap_sec=2.0):
    ...
    for e in events:
        ...
        except (ValueError, KeyError, TypeError): ts = 0
        e["_ts"] = ts
...
def build_project_data(cache_dir: str, state_path: str) -> dict:
    ...
    raw_stats = {}
    try: raw_stats = json.loads(_stats.read_text(encoding="utf-8")) if _stats.exists() else {}
    ...
    wf = {}
    try: wf = json.loads(_state.read_text(encoding="utf-8")) if _state.exists() else {}
    except (json.JSONDecodeError, OSError): pass
