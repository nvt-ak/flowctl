...
Usage: flowctl audit-tokens [--days N] [--step N] [--format table|markdown|json] [--limit N] [--skill-sizes]
"""
...
SKILL_MANIFEST = REPO_ROOT / ".cursor" / "skills" / "core" / "manifest.json"
...
def print_skill_sizes() -> None:
    """P3: line counts for compact SKILL.md vs lazy reference files (manifest-driven)."""
    if not SKILL_MANIFEST.is_file():
        print(f"{yel('Missing manifest:')} {SKILL_MANIFEST}")
        raise SystemExit(1)
    data = json.loads(SKILL_MANIFEST.read_text(encoding="utf-8"))
    rows = data.get("skills_with_detail", [])
    print(f"\n{bold('═══ Core skills — compact vs lazy (lines) ═══')}")
    print(f"  Manifest : {SKILL_MANIFEST.relative_to(REPO_ROOT)}")
    print()
    print(f"  {'id':<24} {'SKILL.md':>10}  {'lazy total':>12}  {'#frags':>7}")
    print(f"  {'─' * 24} {'─' * 10}  {'─' * 12}  {'─' * 7}")
    total_c = total_l = 0
    for e in rows:
        sid = str(e.get("id", "?"))
        c_rel = e.get("compact")
        lazy = e.get("lazy")
        if not c_rel or not lazy or not isinstance(lazy, list):
            print(f"  {red(sid):<24}  (missing compact/lazy)")
            continue
        cp = REPO_ROOT / c_rel
        lc = len(cp.read_text(encoding="utf-8").splitlines()) if cp.is_file() else -1
        ld = 0
        nf = 0
        for rel in lazy:
            dp = REPO_ROOT / rel
            if dp.is_file():
                ld += len(dp.read_text(encoding="utf-8").splitlines())
                nf += 1
            else:
                ld = -1
                break
        ...
        if ld >= 0:
            total_l += ld
        c_s = str(lc) if lc >= 0 else red("MISSING")
        l_s = str(ld) if ld >= 0 else red("MISSING")
        n_s = str(nf) if ld >= 0 else "—"
        print(f"  {sid:<24} {c_s:>10}  {l_s:>12}  {n_s:>7}")
    print(f"  {'─' * 24} {'─' * 10}  {'─' * 12}  {'─' * 7}")
    print(f"  {'TOTAL (listed skills)':<24} {total_c:>10}  {total_l:>12}")
    print()
    print(f"  {cyn('Tip:')} load `SKILL.md` first; open only the lazy reference file(s) listed in that hub (or manifest `lazy`).")
    print()
...
def main() -> None:
    ...
    parser.add_argument(
        "--skill-sizes",
        action="store_true",
        help="Print line counts for compact vs lazy references (from .cursor/skills/core/manifest.json)",
    )
